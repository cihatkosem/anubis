...
module.exports = {
    ...
    description: 'Bir kişinin ses kanalına gitmek için kullanabilirsiniz.',
    run: async (client, command, message, args) => {
        ...
        if (config.admins.includes(message.author.id) || config.moveAdminRoles.some(r => message.member.roles.cache.has(r))) {
            message.member.voice.setChannel(memberVoiceChannel.id)
                .then(() => message.reply({ content: `\`✅\` Seni <#${memberVoiceChannel.id}> kanalına çektim.` }))
                .catch(() => message.reply({ content: "\`❌\` Etiketlediğin üyenin yanına seni çekemiyorum." }));
            return;
        }

        const verifyId = `gonear_command-verify-${message.author.id}-${member.user.id}-${authorVoiceChannel.id}`;
        const cancelId = `gonear_command-cancel-${message.author.id}-${member.user.id}-${authorVoiceChannel.id}`;
        ...
        const msg = await message.reply({ 
            content: 
                `\`❓\` ${member}, ${message.member} adlı üye sizin yanınıza yani <#${authorVoiceChannel.id}> kanalına gelmek istiyor. Gelmesini istiyor musun? \n` +
        ...
        return setTimeout(() => {
            ...
            msg.edit({ 
                content: `\`⌛\` ${member}, ${message.member} adlı üyenin yanına yani <#${authorVoiceChannel.id}> kanalına gitme isteğiniz zaman aşımına uğradı!`,
                components: []
            }).catch((e) => error(e));
        }, 15000);
    }
